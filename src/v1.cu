...
__global__ void odeKernel(double *E, double *R,
...
__global__ void pdeKernel(double *E, double *E_prev, const double alpha, const int n, const int m) {
	int i = threadIdx.x + 1;
	int j = blockIdx.x + 1;
	int index = j * (n + 2) + i;
...
void simulate(double *E, double *E_prev, double *R,
	...
	pdeKernel<<<m, n>>>(E, E_prev, alpha, n, m);
	odeKernel<<<m, n>>>(E, R, n, m, kk, dt, a, epsilon, M1, M2, b);
}
