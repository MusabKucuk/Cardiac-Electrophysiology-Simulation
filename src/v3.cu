...
__global__ void singleKernel(double *E, double *E_prev, double *R,
    ...
    E_temp = E_prev[index] + alpha * (E_prev[index + 1] + E_prev[index - 1] - 4 * E_prev[index] + E_prev[index + m + 2] + E_prev[index - (m + 2)]);
...
void simulate(double *E, double *E_prev, double *R,
	...
	singleKernel<<<m, n>>>(E, E_prev, R, n, m, kk, dt, a, epsilon, M1, M2, b, alpha);
}
